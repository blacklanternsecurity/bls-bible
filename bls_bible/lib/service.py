...
class BibleService:
	...
	def __init__(self):
		...
		self.localDeployment = self.config_management.load_app_config()["localDeployment"]
		...
		# config management
		# updates
		self.update = update(self.source, self.redParent, self.blueParent, self.purpleParent, self.apocryphaParent, self.assessmentParent, self.localPath, self.ttpParent, self.localDeployment, self.parent, self.safePath, self.projectId, self.branch, self.domain, self.token)
		self.mitre_update = mitre_update(self.localPath)
		self.git_update = git_update(self.token, self.domain, self.repo, self.projectId, self.branch, self.parent, self.localPath)
		...
		self.analyze_profiles = analyze_profiles(localPath=self.localPath, localDeployment=self.localDeployment,
													safePath=self.safePath, parent=self.parent)
		# File Editing
		self.file_editing = file_editing(self.localPath, self.localDeployment, self.parent, self.safePath)
	# lib.cli.api_handler
	...
	def new_app_config(self, newToken, newDomain, newRepo, newId, newBranch, newParent, newSource, newLocalDeployment):
		return self.config_management.new_app_config(newToken, newDomain, newRepo, newId, newBranch, newParent, newSource, newLocalDeployment)
	...
	def search_local(self, term, osTags, onpremTags, cloudTags, applicationTags, specialTags, guideTtpTags, aptTags):
		return self.search.search_local(term, osTags, onpremTags, cloudTags, applicationTags, specialTags, guideTtpTags, aptTags)

	def search_remote(self, term):
		return self.search.search_remote(term)
	...
	def git_assessalonians_pull(self):
		return self.git_update.git_assessalonians_pull(self.Assessalonians_Repo, self.assessmentParent)
	...
	def get_content(self, path):
		return self.utils.get_content(path, self.safePath, self.localDeployment, self.parent)

	def get_content_remote(self, path, name):
		return self.utils.get_content_remote(path, name)
	...
	def get_guides_referencing_technique(self, technique):
		return utils.get_guides_referencing_technique(technique)

	def parseJsonData(self, json, type):
		return ui.parseJsonData(self, json, type)

	def getAccordion(self, file, type):
		return ui.getAccordion(self, file, type)
