...
class utils:
	...
	def parse_dir_local(self, parent):
		...
		for idx, folder in enumerate(dirArr):
			...
			if idx < len(dirArr) - 1 or len(fileArr) > 0:
				content += '],\"name\": \"' + folder.replace("_", " ").replace("\\",
																			   "\\\\") + '\",\"type\": \"folder\"},'
			else:
				content += '],\"name\": \"' + folder.replace("_", " ").replace("\\",
																			   "\\\\") + '\",\"type\": \"folder\"}'
		for idx, file in enumerate(fileArr):
			if idx < len(fileArr) - 1:
				content += '{\"name\": \"' + file.replace("_", " ").replace("\\", "\\\\") + \
						   '\",\"type\": \"url\",\"url\": \"' + "getContent('" + top + file + "')" + '\"},'
			else:
				content += '{\"name\": \"' + file.replace("_", " ").replace("\\", "\\\\") + \
						   '\",\"type\": \"url\",\"url\": \"' + "getContent('" + top + file + "')" + '\"}'
		return content
	...
	def get_ttp_content(self, file):
		...
		if not found:
			return False
		ttp = {'phases': phases, 'platforms': platforms, 'name': name, 'description': description,
			   'detection': detection, 'mitreSource': mitreSource, 'allSources': allSources}
		return ttp
	...
	def get_content(self, path, safePath, localDeployment, parent):
		if os.path.commonprefix((os.path.realpath(path), safePath)) != safePath:
		...
		content += '<div onclick="openInNewTab(\'' + str(
			markupsafe.escape(path)) + '\')" class="material-icons col-sm-1">open_in_new</div>'
		if localDeployment:
			content += '<div id="file-edit-btn" onclick="editFile(\'' + str(
				markupsafe.escape(path)) + '\')" class="material-icons col-sm-1">edit</div>'
			content += '<div id="file-save-btn" onclick="saveFile(\'' + str(
				markupsafe.escape(path)) + '\')" style="display:none;" class="material-icons col-sm-1">save</div>'
			content += '<div id="file-undo-btn" onclick="cancelEditFile(\'' + str(
				markupsafe.escape(path)) + '\')" style="display:none;"' \
					   ' class="material-icons col-sm-1">undo</div>'
			content += '<div class="col-sm-5"></div>'
		else:
			content += '<div class="col-sm-8"></div>'
		content += '</div><div class="row">'
		content += '<span class="col-sm-12 file-content-header"><h4><b>' + \
				   str(markupsafe.escape(path.replace(safePath, "/"))) + '</b></h4></span>'
		content += '</div></div>'
		...
		if len(os.path.basename(path).split('.')) > 2:
			filename = os.path.basename(path).split('.')[0] + '.' + os.path.basename(path).split('.')[1]
		else:
		...
		if re.search('(T[0-9]{4}\.{1}[0-9]{3}|T[0-9]{4})', filename):
			...
			content += '<h1>' + str(markupsafe.escape(filename)) + " - " + ttp['name'] + '</h1>'
			content += '</a>'
			# Description
			mdDescription = markdown2.markdown(ttp['description'], extras=["task_list", "fenced-code-blocks", "target-blank-links"])
			...
			for file in files:
				guideRefcontent += '<li>'
				guideRefcontent += '<a id="local-a" href="javascript:undefined" onclick="getContent(\'' + file['path'] + '\')">'
			...
			mdDetection = markdown2.markdown(ttp['detection'], extras=["task_list", "fenced-code-blocks", "target-blank-links"])
		...
		try:
			...
			if path[-1] == "d" and path[-2] == "m":
				mdContent = markdown2.markdown(fileContent,
												extras=[
													"task_list",
													"fenced-code-blocks",
													"header-ids",
					...
					if link.get('href'):
						if len(link.get('href')) >= 4:
							if link.get('href')[0] != 'h' and link.get('href')[1] != 't' and link.get('href')[
								...
								link['onclick'] = "getContent('" + self.localPath + parent + link.get(
									'href').replace("../", "") + "')"
				...
				try:
					...
					finalContent += '<div class="col-sm-2 toc-sidebar">' + toc_content + guideRefcontent + '</div>'
				...
				mdContent += '\n```'
				html = markdown2.markdown(mdContent, extras=["fenced-code-blocks", "task_list"])
...
class utils_remote:
	...
	def parse_dir_remote(self, domain, projectId, token, parent, branch):
		gl = gitlab.Gitlab(domain, private_token=token, ssl_verify=False)
		content = ''
		children = gl.projects.get(self.projectId).repository_tree(parent, self.branch, all=True)
		for idx, child in enumerate(children):
			ch_path = child['path'].replace("\\", "\\\\")  # repo URL (full, not relative to location)
			ch_name = child['name'].replace("\\", "\\\\")  # name of file/folder
			ch_type = child['type']  # can be 'tree' or 'blob' - if 'tree', it is a folder, if 'blob' then it is a file
			if ch_type == 'tree':
				content += '{\"children\": ['
				content += self.parse_dir_remote(ch_path)
				if idx < len(children) - 1:
					content += '],\"name\": \"' + ch_name.replace("_", " ") + '\",\"type\": \"folder\"},'
				else:
					content += '],\"name\": \"' + ch_name.replace("_", " ") + '\",\"type\": \"folder\"}'
			elif ch_type == 'blob':
				if idx < len(children) - 1:
					# TODO: CHANGE THESE TO A getContentRemote() and pull the file off gitlab?
					content += '{\"name\": \"' + ch_name.replace("_", " ") + \
							   '\",\"type\": \"url\",\"url\": \"' + \
							   "getContentRemote('" + ch_path + "', '" + ch_name + "')" + '\"},'
				else:
					content += '{\"name\": \"' + ch_name.replace("_", " ") + \
							   '\",\"type\": \"url\",\"url\": \"' + \
							   "getContentRemote('" + ch_path + "', '" + ch_name + "')" + '\"}'
	...
	def get_content_remote(self, path, name):
		fileContent = ''
		project = self.gl.projects.get(self.projectId)
		items = project.repository_tree(path, self.branch)
		ident = ''
		for item in items:
			if item['name'] == name:
				ident = item['id']
				break
		fileContent = project.repository_raw_blob(ident)
		...
		content = '<div class="modal-header">'
		content += '<div class="row">'
		content += '<span onclick="$(\'#file-content\').modal(\'hide\');" class="material-icons col-sm-1">cancel</span>'
		content += '<span onclick="history.back();" class="material-icons col-sm-1">arrow_back</span>'
		content += '<span onclick="history.forward();" class="material-icons col-sm-1">arrow_forward</span>'
		content += '<span class="col-sm-9"></span>'
		content += '<span style="color:red; padding-top: 10px;" class="col-sm-12"><h4><b>' + \
				   path + name + '</b></h4></span>'
		content += '</div></div>'
		...
		if name[-1] == "d" and name[-2] == "m" and name[-3] == ".":
			mdContent = markdown2.markdown(fileContent, extras=["fenced-code-blocks", "task_list"])
			soup = BeautifulSoup(mdContent, 'html.parser')
			for link in soup.find_all('a'):
				if link.get('href')[0] != 'h' and link.get('href')[1] != 't' and link.get('href')[2] != 't' and \
						link.get('href')[3] != 'p':
					link['onclick'] = "getContentRemote('" + link.get('href').replace("../", "") + "', '" + link.get(
						'href')[link.get('href').rfind("/") + 1:] + "')"
					link['href'] = 'javascript:undefined'
					link['id'] = "local-a"
				else:  # where we're going we don't need roads
					link['target'] = "_blank"
			content += str(soup)
		content += '</div>'
		return content
