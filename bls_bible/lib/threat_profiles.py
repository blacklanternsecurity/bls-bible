...
class manage_profiles:
	...
	def getGroups(self):
		...
		for profile in data:
			...
			content += '<div class="col-sm-1">' \
					   ...
					   'threat-profile-check" value="' + profile['id'] + '"></div>'
			content += '<div class="col-sm-8">' + profile['name'] + '</div>'
			content += '<span onclick="editGroup(\'' + profile['id'] + \
					   '\')" class="material-icons col-sm-1 no-border">edit</span>'
			content += '<span onclick="deleteGroup(\'' + profile['id'] + \
				...
				if profile['ttps']:
					...
					for ttp in profile['ttps']:
						if not ttp['ttp_minor'] == '':
							content += '<li>' + ttp['ttp_file'] + ' - ' + ttp['ttp_major'] + \
									   ' - ' + ttp['ttp_minor'] + '</li>'
						else:
							content += '<li>' + ttp['ttp_file'] + ' - ' + ttp['ttp_major'] + '</li>'
				content += '</ul>'
				content += '</div>'
			content += '</div>'
		return content
	...
	def getGroupForEdit(self, id):
		...
		content += '<input id="profile_name" class="form-control bg-dark text-light" type="text" value="' + \
				   group['name'] + '">'
		...
		if len(group['ttps']) > 0:
			...
			for ttp in group['ttps']:
				if not ttp['ttp_minor'] == '':
					content += '<li data-ttp-file="' + ttp['ttp_file'] + '" data-ttp-major="' + ttp['ttp_major'] + \
							   '"  data-ttp-minor="' + ttp['ttp_minor'] + '" class="group-edit-ttp">' + \
							   ttp['ttp_file'] + ' - ' + ttp['ttp_major'] + ' - ' + ttp['ttp_minor'] + \
		...
		content += '<span onclick="updateGroup(\'' + group['id'] + \
				   '\')" class="col-sm-2 material-icons">save</span>'  # col 2
	...
	def getGroupsForAddProfile(self, path):
		...
		content += '<li class="context-menu-item add-to-profile-item" onclick="addTTPToProfile(\'' + \
				   str(markupsafe.escape(path)) + '\', \'new\')">Add To New Profile</li>'
		for profile in data:
			content += '<li class="context-menu-item add-to-profile-item" onclick="addTTPToProfile(\'' + \
					   str(markupsafe.escape(path)) + '\', \'' + profile['id'] + '\')">' + profile['name'] + '</li>'
		return content
...
class analyze_profiles:
		...
		for selected in selected_profiles:
			...
			for custom in custom_profiles:
				...
				if len(custom_techniques) == 0:
					...
					percentage = str(round(counter / len(custom_techniques) * 100, 2)) + "%"
				if counter == 0:
					continue
			...
			for apt in mitre_apts:
				...
				if len(apt_techniques) == 0:
					...
					percentage = str(round(counter / len(apt_techniques) * 100, 2)) + "%"
				if counter == 0:
					continue
	...
	def export_profile_to_pdf(self, profileIds):
		...
		for filePath in filePaths:
			html_doc += self.utils.get_content(filePath, self.safePath, self.localDeployment, self.parent)
...
class CLI_manage_profiles:
	...
	def createProfile_ETM(self,
					name="New Threat Profile",
					ttp=None,
					ETM_Scenario_id=None):
	...
	def TTP_lookup_in_MITRE(self, mitre_TTP_value):
		...
		if (re.match("^T[0-9]{4}\.[0-9]{3}$", mitre_TTP_value)) or (re.match("^T[0-9]{4}$", mitre_TTP_value)):
			ttp = mitre_TTP_value
			...
			if (re.match("^T[0-9]{4}/[0-9]{3}$", mitre_TTP_value)):
				...
				print("mitre TTP id format incorrect. Expecting either \"T1234.001\" or \"T1234\" format")
...
#		ttp_tech = ttp_file.replace('.md', '').split('_')[0]
	...
	def addTTPToProfile_ETM(self, mitreId, profileId, threat_source, step_name, finding, ETM_Step_id=None):
		...
		if mitreId is None:
			...
			if (re.match("^T[0-9]{4}\.[0-9]{3}$", mitreId)) or (re.match("^T[0-9]{4}$", mitreId)):
				ttp = mitreId
				print("first ttp occurance")
			else:
	...
	def updateTTPs_ETM(self, ttp_id, step_name=None, ttp=None, finding=None, threat_source=None, ETM_Step_id=None):
		...
		if ttp is None:
			...
			if (re.match("^T[0-9]{4}\.[0-9]{3}$", ttp)) or (re.match("^T[0-9]{4}$", ttp)):
				ttp = ttp
				print("first ttp occurance")
			else:
