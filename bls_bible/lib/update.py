...
class update:
	def __init__(self, source, redParent, blueParent, purpleParent, apocryphaParent, assessmentParent, localPath, ttpParent, localDeployment, parent, safePath, id, branch, domain, token):
		...
		self.utils_remote = utils_remote()
	def update_json_file(self):
		# Red:
		content = ''
		content += '{\"children\": ['
		if self.source == "remote":
			content += self.utils_remote.parse_dir_remote(self.domain, self.projectId, self.token, self.parent, self.branch)
		else:
		...
		try:
			f = open("bls_bible/static/red.json", 'w+')
		...
		json_formatted_str = json.dumps(json_object, indent=2)
		try:
			f = open("bls_bible/static/blue.json", 'w+')
			f.write(json_formatted_str)
		finally:
			f.close()
		...
		if os.path.exists(self.localPath + self.assessmentParent):
			...
			if self.source == "remote":
				...
				content += self.utils.parse_dir_local(self.localPath + self.assessmentParent)
		...
		#
		# if self.source == "remote":
		#	content += self.utils.parse_dir_remote(self.parent)
		# else:
		#	content += self.utils.parse_dir_local(self.localPath + self.ttpParent)
		# content += '],\"name\": \"TTP\",\"type\": \"folder\"}'
		...
		for obj in mitre['objects']:
			if obj['type'] == 'x-mitre-tactic':
				phase = {
					"children": [],
					"name": obj['external_references'][0]['external_id'] + ' ' + obj['name'],
					"type": 'folder',
							...
							if ttp_is_sub:
								...
								for obj2 in mitre['objects']:
									if obj2['type'] == 'attack-pattern' \
											and obj2['external_references'][0]['external_id'] == file.split('.')[0]:
					...
					if phase['short_name'] == ttp_phase:
						if ttp['is_sub']:
							...
							for parent in phase['children']:
								if parent['type'] == 'folder' and parent['name'] == ttp['technique'].split('.')[0] + \
										' ' + ttp['parent_title']:
									parentExists = True
									# Check to see if child folder already exists
									childExists = False
									for child in parent['children']:
										if child['type'] == 'folder' and child['name'] \
												== ttp['technique'].split('.')[1] + ' ' + ttp['sub_title']:
									...
									if not childExists:
										...
										new_child = {
											'children': [new_file],
											'name': ttp['technique'].split('.')[1] + ' ' + ttp['sub_title'],
											'type': 'folder'
										}
							...
							if not parentExists:
								...
								new_parent = {
									...
									'name': ttp['technique'].split('.')[0] + ' ' + ttp['parent_title'],
									'type': 'folder'
								}
							...
							parentExists = False
							for parent in phase['children']:
								if parent['type'] == 'folder' and parent['name'] == \
							...
							if not parentExists:
								...
								new_parent = {
									'children': [new_file],
									'name': ttp['technique'] + ' ' + ttp['parent_title'],
									'type': 'folder'
								}
		...
		for phase in ttp_data['children']:
			phase['children'].sort(key=lambda tech: ' '.join(tech['name'].split(' ')[1:]))
					...
					for obj2 in mitre['objects']:
						if obj2['type'] == 'attack-pattern' \
							and obj2['external_references'][0]['external_id'] == \
								obj['external_references'][0]['external_id'].split('.')[0]:
									ttp_parent_title = obj2['name']
									break
				else:
					ttp_parent_title = obj['name']
		...
		for ttp in ttp_arr:
			try:
				...
				print(e)
				continue
		with open(self.localPath + 'bls_bible/static/missing_techniques.json', 'w+') as out:
...
class mitre_update:
	...
	def get_apts(self):
		...
		for profile in profiles:
			...
			content += '<input onchange="toggleAptUiFilter(this)" class="form-check-input apt-checkbox" ' \
	...
	def crawlGroups(self):
		...
		for value in groupsSorted:
			groupsDictSorted[list(groups.keys())[list(groups.values()).index(value)]] = value
		return groupsDictSorted

	def crawlJson(self, id):
		url = "https://attack.mitre.org/groups/" + id + "/" + id + "-enterprise-layer.json"
		r = requests.get(url)
		try:
...
class autoupdate:
	def __init__(self):
		pass
	def schedule_update():
#		schedule.every(1).minute.do(print(git_update))
#		schedule.every().day.at("01:00").do(git_update())
		print("reached schedule_update")
#		while True:
