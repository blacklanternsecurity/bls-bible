...
class ui:
		...
		if arr:
			...
			for file in uniqFiles:
				...
				for phase in technique['phases']:
					finalFiles.append(file.split('/')[0] + "/" + phase['name'].replace(' ', '_') +
									  '/' + '/'.join(file.split('/')[1:]))
	...
	def getTabContent(self, path):
		...
		if re.search('(T[0-9]{4}\.{1}[0-9]{3}|T[0-9]{4})', filename):
			...
			content += '</a>'
			# Description
			mdDescription = markdown2.markdown(ttp['description'],
			...
			for file in files:
				content += '<li>'
				content += '<a id="local-a" href="javascript:undefined" onclick="getContent(\'' + file['path'] + '\')">'
				content += file['name']
				content += '</a>'
				content += '</li>'
			content += '</ul>'
		...
		if path[-1] == "d" and path[-2] == "m":
			mdContent = markdown2.markdown(fileContent,
										   extras=["task_list", "fenced-code-blocks", "header-ids", "tables", "toc"])
			soup = BeautifulSoup(mdContent, 'html.parser')
			...
			for link in soup.find_all('a'):  # adjusting Links for relative paths and external paths
				if len(link.get('href')) >= 4:
					if link.get('href')[0] != 'h' and link.get('href')[1] != 't' and link.get('href')[
						...
						link['onclick'] = "openInNewTab('" + self.localPath + self.parent + link.get(
							'href').replace("../", "") + "')"
			...
			b_tag.append(str(markupsafe.escape(path.split(self.localPath+self.parent)[1])))
	...
	def parseJsonData(self, json, type):
		...
		for child in json:
			self.accordionKey += 1
			accordionId = hashlib.md5((type + child['name'] + str(self.accordionKey)).encode()).hexdigest()
			if child['type'] == 'folder':
				...
				content += '<h2 data-ttp-toggle-count="0" data-blue-toggle-count="0" data-red-toggle-count="0" ' \
						   'class="accordion-header" id="panelsStayOpen-heading' + accordionId + '">'
				content += '<button class="accordion-button collapsed" type="button" ' \
						   'data-bs-toggle="collapse" data-bs-target="#panelsStayOpen-collapse' + accordionId + \
						   '" aria-expanded="false" aria-controls="panelsStayOpen-collapse' + accordionId + '">'
				content += child['name']
				content += '<span data-toggle-count="0" class="material-icons no-border highlighter-' + type + '">highlight</span>'
				content += '</button>'
				content += '</h2>'
				content += '<div id="panelsStayOpen-collapse' + accordionId + \
				...
				if type.lower() == "red" or type.lower() == "blue":
					content += '<span oncontextmenu="context(event, \'' + filePath + '\', \'' + \
							   type.lower() + '\', this)" data-ttp-toggle-count="0" onclick="' + \
							   child['url'] + '" class="accordion-header file ' + indent + \
							   '" id="panelsStayOpen-heading' + accordionId + '">'
				else:
					content += '<span oncontextmenu="context(event, \'' + filePath + '\', \'' + type.lower() + \
							   '\', this)" data-blue-toggle-count="0" data-red-toggle-count="0" onclick="' + \
							   child['url'] + '" class="accordion-header file ' + indent + \
							   '" id="panelsStayOpen-heading' + accordionId + '">'
				content += child['name']
				content += '<span data-toggle-count="0" class="material-icons no-border highlighter-' + \
						   type + '">highlight</span>'
				content += '</span>'
				content += '</div>'
				content += '</div>'
	...
	def getAccordion(self, file, type):
		...
		try:
			...
			return ''
		f.close()
		accordionId = hashlib.md5((type + data['name'] + str(self.accordionKey)).encode()).hexdigest()
		content = ''
		content += '<div class="accordion" id="accordionPanelsStayOpenExample">'
		content += '<div class="accordion-item">'
		content += '<h2 class="accordion-header" id="panelsStayOpen-heading' + accordionId + '">'
		content += '<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" ' \
				   'data-bs-target="#panelsStayOpen-collapse' + accordionId + \
				   '" aria-expanded="false" aria-controls="panelsStayOpen-collapse' + accordionId + '">'
		content += data['name']
		content += '</button>'
		content += '</h2>'
