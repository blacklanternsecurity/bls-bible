...
class developer_tools:
        ...
        for missing in data:
            ...
            if data[missing]["sub_title"] == "":
                content += (
                    ...
                    + missing
                    + "_"
                    + data[missing]["parent_title"].replace(" ", "_")
                    + "/"
                    + missing
                    + ".md"
                )
                ...
                content += (
                    ...
                    + missing.split(".")[1]
                    + "_"
                    + data[missing]["sub_title"].replace(" ", "_")
                    + "/"
                    + missing
                    + ".md"
                )
    ...
    def get_guides_not_referencing(self):
        ...
        try:
            ...
            logging.debug("reverse_reference.json appears to be empty")
            pass
        content = (
            "<html>"
            "<head>"
        ...
        for guide in red_guides:
            content += (
                "<li>" + guide["guide_name"] + " : " + guide["guide_path"] + "</li>"
            )
        content += "</ul>" "</details>" "</div></div>"
        content += (
            '<div class="row"><div class="col-sm-12">'
            "<details>"
    ...
    def validate_folder_naming(self):
        ...
        for ref in ref_dict:
            ...
            if (
                technique["parent_title"]
                .replace("/", "-")
                .replace(" ", "_")
                .replace(")", "")
                .replace("(", "")
                not in technique["full_path"]
            ):
                invalid = True
                result["parent-old"] = "_".join(
                    technique["full_path"].split("/")[1].split("_")[1:]
                )
            ...
            if (
                ...
                invalid = True
                try:
                    result["sub-old"] = "_".join(
                        technique["full_path"].split("/")[2].split("_")[1:]
                    )
        ...
        for entry in invalid_folder_names:
            ...
            content += "<details>" "<summary>" + entry["full-path"] + "</summary>"
            content += "<ul>"
            content += "<li>Old Parent: " + entry["parent-old"] + "</li>"
            content += (
                "<li>New Parent: " + entry["parent-new"].replace(" ", "_") + "</li>"
            )
    ...
    def get_missing_guide_references(self):
        ...
        try:
            ...
            logging.debug(
                "devtools.get_missing_guide_references attempted to del"
                " a dictionary entry for an empty dictionary"
            )
            pass
        ...
        for technique in trim_dict:
            ...
            for platform in tech["platforms"]:
                ...
                for guide in tech["guides_referencing"]:
                    if platform == guide["guide_os"] and guide["guide_color"] == "red":
                        ...
                        platform == guide["guide_os"] and guide["guide_color"] == "blue"
                    ):
                        has_blue = True
                if not has_red:
            ...
            for guide in tech["guides_referencing"]:
                if (
                    guide["guide_os"] not in tech["platforms"]
                    and guide["guide_os"] != ""
                ):
            ...
            if len(tech["missing_red"]) > 0:
                ...
                if tech["is_sub"]:
                    link_str = (
                        ...
                        + technique.split(".")[0]
                        + "_"
                        + tech["parent_title"].replace(" ", "_")
                        + "/"
                        + technique.split(".")[1]
                        + "_"
                        + tech["sub_title"].replace(" ", "_")
                        + "/"
                        + tech["file_name"]
                        + "))"
                    )
